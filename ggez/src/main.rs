...
fn rand_color(rng: &mut Rand32) -> Color {
    let r: u8 = rng.rand_range(0..255) as u8;
    let g: u8 = rng.rand_range(0..255) as u8;
    let b: u8 = rng.rand_range(0..255) as u8;
...
impl GameState {
    fn new(ctx: &mut Context) -> ggez::GameResult<GameState> {
        ...
        let texture = Image::from_path(ctx, "/assets/wabbit_alpha.png")?;
    ...
    fn draw(&mut self, ctx: &mut Context) -> GameResult {
        ...
        if self.batched_drawing {
            self.bunnybatch.set(
                self.bunnies
                    .iter()
                    .map(|bunny| graphics::DrawParam::new().dest(bunny.position).color(bunny.color)),
            );
...
fn main() -> GameResult {
    ...
    let cb = ggez::ContextBuilder::new("bunnymark", "ggez").add_resource_path(resource_dir).window_setup(
